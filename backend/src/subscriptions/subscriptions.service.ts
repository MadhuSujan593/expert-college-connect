...
export class SubscriptionsService {
  ...
  async confirmPayment(userId: string, planId: string, paymentData: any, billingPeriod: string = 'MONTHLY') {
    try {
      ...
      // For free plans, ignore billing period and use plan duration (defaulting to 30 days if not set)
      if (plan.priceCents === 0) {
        const durationDays = plan.durationDays || 30;
        endsAt = new Date(now.getTime() + (durationDays * 24 * 60 * 60 * 1000));
        console.log(`Free plan activation: Setting expiration to ${durationDays} days from now (${endsAt.toISOString()})`);
